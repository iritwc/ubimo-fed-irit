...
import {
    debounceTime, distinctUntilChanged, switchMap, timeInterval
} from 'rxjs/operators';
...
export interface IAdEventData extends IAdEvent {
    time: number;
    show: boolean;
}
...
export class AdDispatcher {
    ...
    private _adIntervalSubscriber;

    constructor() {
        // this.startEmissions();
    }

    getRandomAd(): IAdEvent {
    ...
    startEmissions(): void {
        this._adIntervalSubscriber = interval(5000).subscribe((x) =>
            this._adDispatcher$.next(this.getRandomAd()));
    }
    ...
    registerToAdEvents(cb: (adEvent: IAdEvent) => void): { removeListener: () => void } {
        ...
        return {
            removeListener: () => {
                if (this._adIntervalSubscriber) { this._adIntervalSubscriber.unsubscribe();}
                sub.unsubscribe();
            }
        };
    }
